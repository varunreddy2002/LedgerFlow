...
class Account(Base, PrimaryKeyMixin, CreatedAtMixin):
...
class Category(Base, PrimaryKeyMixin, CreatedAtMixin):
    ...
    # remote_side uses string "Category.id" because `id` lives on PrimaryKeyMixin,
    # not in this class body — a plain [id] would resolve to Python's built-in id().
...
class Vendor(Base, PrimaryKeyMixin, CreatedAtMixin):
    ...
    normalized_name: Mapped[Optional[str]] = mapped_column(String(150), index=True)
...
class Customer(Base, PrimaryKeyMixin, CreatedAtMixin):
